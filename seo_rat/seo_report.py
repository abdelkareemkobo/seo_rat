...
# %% auto #0
__all__ = ['path', 'website_name', 'start', 'end', 'sync_articles_to_db', 'find_duplicate_metadata', 'analyze_links',
...
with db.get_session() as session:
    ...
    for article in articles: 
        ...
        print(page_path)
        top_query = get_top_queries(session=session,site_url="sc-domain:awazly.com",start_date=start,end_date=end,page_path=page_path,limit=1,sort_by="impressions")
        top_5_query = get_top_queries(session=session,site_url="sc-domain:awazly.com",start_date=start,end_date=end,page_path=page_path,limit=5,sort_by="impressions")
        try:
...
def print_issues_report(report: dict) -> None:
    ...
    for url, data in sorted_issues:
        ...
        if missing:
            ...
            for q in missing:
                print(f"      - {q['query']} (clicks: {q['total_clicks']}, impressions: {q['total_impressions']}, pos: {q['avg_position']:.1f})")
