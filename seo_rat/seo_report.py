...
# %% auto #0
__all__ = ['sync_articles_to_db', 'find_duplicate_metadata', 'analyze_links', 'check_heading_structure', 'analyze_article',
           'generate_seo_report', 'print_issues_report']
...
def analyze_article(article: Article,  # Article to analyze
...
def _collect_page_issues(report: dict,  # Result from `analyze_article`
                         article: Article  # Article being analyzed
                         ) -> list[str]:
    "Collect all SEO issues from an article report as a list of strings."
    issues = []
    if not report["h1_check"]["has_single_h1"]: issues.append("Multiple or no H1")
    if not report["content_check"]["is_sufficient"]: issues.append("Content too short")
    if report["link_analysis"]["internal_count"] == 0: issues.append("No internal links, at least 3 recommended")
    if report["link_analysis"]["external_count"] == 0: issues.append("No external links")
    if report["missing_alts"]: issues.append(f"Images missing alt text ({len(report['missing_alts'])})")
    if not report["content_structure"]["has_h2"]: issues.append("No H2 headings, at least one recommended")
    ...
    if tc["status"] != "optimal": issues.append(
        f"Title {tc['status']} ({tc['length']} chars, range {tc['min']}-{tc['max']})")
    dc = report["description_check"]
    if dc["status"] != "optimal": issues.append(
        f"Description {dc['status']} ({dc['length']} chars, range {dc['min']}-{dc['max']})")
    if not report["content_freshness"]["is_fresh"]: issues.append(
        f"Stale content ({report['content_freshness']['days_since_update']} days old)")
    if article.focus_keyword:
        if not report["keyword_placement"]["in_h1"]: issues.append("Focus keyword not in H1")
        if not report["keyword_placement"]["keyword_in_meta"]["in_description"]: issues.append(
            "Focus keyword not in description")
        if not report["keyword_placement"]["keyword_in_meta"]["in_title"]: issues.append("Focus keyword not in title")
        if not report["keyword_placement"]["keyword_at_first"]: issues.append("Focus keyword not in first 100 words")
    return issues


def generate_seo_report(session: Session,
                        website_id: int,
                        domain: str,
                        is_quarto: bool,
    ...
    for article in articles:
        try:
            report = analyze_article(article, domain, is_quarto,
                                     title_is_h1=title_is_h1, desc_field=desc_field,
                                     title_field=title_field, date_field=date_field)
            queries = get_top_queries(session=session, site_url=f"sc-domain:{domain}",
                                      start_date=start, end_date=end,
                                      page_path=article.url.removeprefix(f"https://{domain}/"),
                                      limit=500, sort_by="impressions")
            ...
            if include_insights:
                ...
                trends = detect_query_trends(session, f"sc-domain:{domain}",
                                             page_path=page_path, days=days, limit=query_limit)
                report["query_intents"] = classify_page_intents(session, f"sc-domain:{domain}",
                                                                insight_start, insight_end,
                                                                page_path=page_path, limit=query_limit)
                report["trends"] = {"rising": [t for t in trends if t["trend"] == "rising"],
                                    "declining": [t for t in trends if t["trend"] == "declining"]}
                report["green_keywords"] = find_green_keywords(session, f"sc-domain:{domain}",
                                                               page_path, content, days=days)

            if page_issues or missing_queries:
                issues[article.url or article.file_path] = {
                    "file_path": article.file_path,
                    "issues": page_issues,
                    "data": {"missing_queries": missing_queries,
    ...
    return {"total_pages": len(articles), "pages_analyzed": len(page_reports),
            "page_reports": page_reports,
            "duplicate_titles": duplicate_titles,
            "duplicate_descriptions": duplicate_descriptions,
            "issues": issues,
            "summary": {"total_issues": sum(len(v["issues"]) for v in issues.values()),
